...
#define REBIN 115//55     //pPb_pt:6,pPb_y:4,pp_pt:8,pp_y:4
#define REBINp 116//56   //pPb_pt:7,pPb_y:5,pp_pt:9,pp_y:5
...
#define REBIN_bin1 5
#define REBIN_bin2 8
#define REBIN_Fine 55
#define BIN_NUM 230 //pPb_pt:220,pPb_y:40,pp_pt:222,pp_y:45

void Bplusdsigmadpt_ATL(int option=5, bool isBinned=true, int bincase=0)
{
  ...
  if (option==72) {getdata.open("../FONLLInputs/fo_pPb_pt_rap24_all_7TeVATL.dat");ename="7TeVATL";}
  //if (option==72) {getdata.open("../FONLLInputs/fo_pPb_pt_rap225_all_7TeVATL.dat");ename="7TeVATLv2";}
  ...
  if (isBinned)
    ...
    {
      isBinnedrmk="Unbinned";REBINn=REBIN;
    }
  ...
  double rebiny_CMS[6] = {5.,10.,13.,17.,24.,30.};//CMS pp binning
  double rebiny_ATL[9] = {9.,13.,16.,20.,25.,35.,50.,70.,120.};//ATLAS pp binning
  ...
  for(j=0;j<REBINn;j++)
    {
      ...
      bin_num[j]=(rebin[j+1]-rebin[j])/0.50;//number of each rebinned bin
  ...
  cr->SetLogy();
  TH2F* hempty=new TH2F("hempty","",10,5,120.,10,0.001,100000000.0);  
  ...
  */
  ...
  TF1* fitft = new TF1("fitft","pow(10,[0]*exp([1]*x)+[2])",5.0,120.0);
  ...
  hempty=new TH2F("hempty","",10,0,130.,10.,0.0001,1000000.0);  
