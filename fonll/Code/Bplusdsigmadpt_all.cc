...
void Bplusdsigmadpt_all(int option, bool isBinned)
{
	...
	TFile*foutput=new TFile(Form("../outputBplus_%s_%s.root",isBinnedrmk.c_str(),ename.c_str()),"recreate");
	...
	leg->Draw("");
	c1->SaveAs(Form("Plots/CompFONLLdsigmadpt_%s_%s.pdf",isBinnedrmk.c_str(),ename.c_str()));

	// REBIN here

	double rebiny[7] = {5,10,15,20,25,30,60};//CM frame

	Double_t rebin[56];
	Float_t apt[55];
	Float_t aptl[55];
	Float_t asigma[55],aminall[55],amaxall[55],aminsc[55],amaxsc[55],aminmass[55],amaxmass[55],aminpdf[55],amaxpdf[55],aerrorl[55],aerrorh[55];
	...
	for(j=0;j<REBINn;j++)
	{
		...
		bin_num[j]=(rebin[j+1]-rebin[j])/0.25;//number of every rebinned bin
...
*/
	...
	TLatex * tlatex;
	tlatex=new TLatex(0.18,0.85,Form("pp collisions at %s from FONLL, 5<p_{T}<60, %1.3f<y_{CM}<%1.3f",tlatexrem.c_str(),tlatexptmin,tlatexptmax));
	...
	cr->SaveAs(Form("Plots/cBmesonPredFONLLBplus%s_%s.pdf",isBinnedrmk.c_str(),ename.c_str()));
...
//	TGraphAsymmErrors* gaeSigmaDecayv2=(TGraphAsymmErrors*)gae->Clone();
	...
	//###TF1* fitft2 = new TF1("fitft2","pow(10,[0]*exp([1]*x)+[2])",5.0,60.0);
	...
	hempty->GetXaxis()->SetLabelSize(0.04);
	hempty->GetYaxis()->SetLabelSize(0.04);  
	hempty->GetYaxis()->SetTitle("d#sigma (B^{+}#rightarrowJ/#psi K^{+})/dp_{T} #times A (pb c/GeV)");
	...
	canvas->SaveAs(Form("Plots/canvasBplus%s_%s.pdf",isBinnedrmk.c_str(),ename.c_str()));
	canvas->Clear();
	//hmaxall_rebinv2->GetYaxis()->SetRangeUser(0.00,2.6*hmaxall_rebinv2->GetMaximum());
	canvas->SetLogy();
	hempty->GetYaxis()->SetTitle("d#sigma (B^{+} full chain)/dp_{T} #times A (#mub c/GeV)");
	//hempty->GetYaxis()->SetRangeUser(1.0,1000000.0);
	...
	canvas->SaveAs(Form("Plots/canvasBplusv2%s_%s.pdf",isBinnedrmk.c_str(),ename.c_str()));
